...
RSpec.describe Task, type: :model do
  ...
  describe 'validations' do
    it 'validates presence of name' do
      task = Task.new(name: nil, description: 'Task Description', status: :todo, project:)
      expect(task).not_to be_valid
      expect(task.errors[:name]).to include("can't be blank")
    end

    it 'validates length of name' do
      task = Task.new(name: 'name' * 15, description: 'Task Description', status: :todo, project:)
    ...
    it 'validates presence of description' do
      task = Task.new(name: 'Task Name', description: nil, status: :todo, project:)
      expect(task).not_to be_valid
    ...
    it 'validates and accept only status: todo, in_progress, completed' do
      [0, 1, 2].each do |valid_status|
        task = Task.new(name: 'Task Name', status: valid_status, description: 'Task Description', project:)
      ...
      [3, 4, 5, 6].each do |invalid_status|
        expect do
          Task.new(name: 'Task Name', status: invalid_status, description: 'Task Description', project:)
  ...
  describe 'status enum' do
    it 'set TODO status by default' do
      task = Task.new(name: 'Task Name', description: 'Task Description', project:)
      task.save
