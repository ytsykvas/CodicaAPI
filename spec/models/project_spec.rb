...
RSpec.describe Project, type: :model do
	...
	describe 'validations' do
		it 'validates presence of name' do
			project = Project.new(name: nil, description: 'Test description', user: user)
			expect(project).not_to be_valid
			expect(project.errors[:name]).to include("can't be blank")
		end

		it 'validates length of name' do
			project = Project.new(name: 'a' * 51, description: 'Test description', user: user)
			expect(project).not_to be_valid
			expect(project.errors[:name]).to include('is too long (maximum is 50 characters)')
		end

		it 'validates presence of description' do
			project = Project.new(name: 'Test Project', description: nil, user: user)
			expect(project).not_to be_valid
